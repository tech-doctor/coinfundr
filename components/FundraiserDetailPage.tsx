          ...
          <div className="w-full mt-[1rem] flex bg-white rounded-[8px] shadow-[0_.2px_30px_4px_rgba(29,48,59,0.04)]">
            ...
            <div className="w-full p-3.5 sm:p-5 md:p-8 flex flex-col justify-between">
              ...
              <div className='mb-8'>
                ...
                <h5 className="font-bold text-sm sm:text-md text-[#5E6364]">&bull; {tag}
                </h5>
            </div>
          </div>
    ...
    >
      ...
      <div className="flex justify-between mt-8 w-full">
        ...
        : <button onClick={handleOpen} className="text-[1rem] font-semibold bg-[#FFC300] text-white px-[1rem] py-[.3rem] sm:py-[.5rem] rounded-[5px] self-start">Donate</button>
         ...
         {page === 'individuals' || page === "search" ? <button onClick={(e) => {sendTransaction?.()}} className="text-[1rem] font-semibold bg-[#0F8E4B] text-white px-[1rem] py-[.3rem] sm:py-[.5rem] rounded-[5px] cursor-pointer">{isLoading ? 'Sending...' : 'Send'}</button>
        : <button onClick={(e) => {
          sendTransaction?.()
          if (isSuccess) {setRedeemNft(true)}
          }}  className="text-[1rem] font-semibold bg-[#0F8E4B] text-white px-[1rem] py-[.3rem] sm:py-[.5rem] rounded-[5px] cursor-pointer">{isLoading ? 'Sending...' : 'Send'}</button>
        }
      </div>
      {isSuccess && 
        <div>
